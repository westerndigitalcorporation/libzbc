...
# Test print
echo "[TEST][${nr_test}][SWRZ][OPEN_ZONE][ALL][CLOSE_TO_EXPLICIT_OPEN][NO_ERROR] start"
...
######################## Get zone info ######################## 
# Get zone info
sudo zbc_test_report_zones -v -ro 3 ${device} > ${nr_test}_zone_info.log 2>&1
...
while read _line; do
    ...
    if [ ${4} = "0x3" -a ${5} = "${test_zone_lba}" ]; then
        target_lba=$(( ${5} + 0 ))
