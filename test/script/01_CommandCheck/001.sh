...
# Test print
echo "[TEST][${nr_test}][REPORT_ZONES][LOGICAL_BLOCK_OUT_OF_LANGE] start"
...
######################## Start Testing ######################## 
# Execution command
sudo zbc_test_report_zones -v -lba $(( ${max_lba} + 1 )) ${device} >> ${nr_test}.log 2>&1
...
if [ ${sk} = ${expected_sk} -a ${asc} = ${expected_asc} ]; then
    ...
    echo "[TEST][${nr_test}][SENSE_KEY],${sk}"
    echo "[TEST][${nr_test}][ASC_ASCQ],${asc}"
fi
