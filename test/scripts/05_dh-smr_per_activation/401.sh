...
zbc_test_init $0 "ZONE ACTIVATE${zbc_test_actv_flags}: deactivate Implicitly-OPEN non-first realm second zone" $*
...
write_lba=$(( ${target_slba} + ${target_size} ))
...
zbc_test_fail_exit_if_sk_ascq "Initial write lba=${write_lba} zone_type=${smr_type}"
