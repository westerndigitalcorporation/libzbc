...
zbc_test_init $0 "ZONE ACTIVATE(32): attempt to deactivate CLOSED partly-written sequential zone (zone addressing)" $*
...
# Make the first zone of the domain non-empty and closed
...
zbc_test_run ${bin_path}/zbc_test_close_zone ${device} ${domain_seq_start}
zbc_test_get_sk_ascq
zbc_test_fail_if_sk_ascq "Initial CLOSE failed at ${domain_seq_start} zone_type=${smr_type}"
