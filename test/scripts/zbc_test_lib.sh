...
function _zbc_test_lib_init()
{
	...
	declare -rg ZT_RESTRICT_READ_INACTIVE="0x2|0x4"	# Read of INACTIVE zones disallowed when !URSWRZ #XXX SWP?
	...
	declare -rg ZA_STAT_MULTI_DOMAINS="0x4020"	# Activation range crosses domain boundary
	declare -rg ZA_STAT_SECURITY="0x4040"		# Security prerequisite unmet
	# declare -rg ZA_STAT_CROSS_DOMAINS="0x4028"	# Crossing domains also gets multiple zone types XXX
	declare -rg ZA_STAT_CROSS_DOMAINS="0x4020"	# Remove in favor of line above XXX
...
# Write a message to the device's error log
function zbc_test_write_dev_log()
{
	if [[ -n ${NO_WRITE_BUFFER} ]]; then
	    return
	fi
	...
	{   # SPC-4 6.49.12 Table 342
	    #        H  G  S  T                                                 ASCII
	    printf "48 47 53 54 20 20 20 20 00 00 00 00 00 00 00 00 00 00 00 00 02 00 00 00 00 %02x" ${sz} | xxd -r -p
	    printf "%s" "${msg}"
	} | sg_write_buffer --mode=0x1c --raw ${device} >& /dev/null
}

# Set the test number into the device's test_attribute (ID=1777h)
function zbc_test_write_test_attribute()
{
	if [[ -n ${NO_WRITE_ATTRIBUTE} ]]; then
	    return
	fi
	...
	len=$(( ${sz} + 9 ))	# header before text string is 9 bytes long

	printf -v CMD "8D 00 00 00 00 00 00 00 00 00 00 00 00 %02x 00 00" ${len}
	{
	    printf "00 00 00 00 17 77 01 00 %02x" ${sz} | xxd -r -p
	    printf "%s" "${test_id}"
	} | sort | sg_raw --send=${len} ${device} ${CMD} >& /dev/null
	#  Note: the "sort" works around a bug in sg_raw
}
...
function zbc_test_case_post_process()
{
	...
	if [ ${failed} -eq 0 ]; then
		...
		# Write the failing test script number to the device's log
		if [[ "${test_case_id}" != "04."* ]]; then
			zbc_test_write_dev_log "FAILED ${test_case_id} ${test_case_desc} -- ${log_file}"
		fi
...
function zbc_test_init()
{
	...
	zbc_test_write_test_attribute "${test_case_id}" # (${log_file})
...
function zbc_read_check_available()
{
	...
	if [ "${_cond}" = "${ZC_OFFLINE}" ]; then
		expected_sk="Data-protect"
		expected_asc="Zone-is-offline"
	elif [ "${_type}" = "${ZT_CONV}" -o "${_type}" = "${ZT_SWP}" ]; then
		return 0
...
function zbc_test_get_zones()
{
	...
	for _line in `zbc_zones | zbc_zone_filter_in_type "${zone_type}"`; do
		...
		zbc_test_get_target_zone_from_slba_or_fail ${slba}
		return 0
	done
...
function zbc_test_search_zone_pair()
{
	...
	for _line in `zbc_zones | zbc_zone_filter_in_type "${zone_type}"`; do
		...
		zbc_test_get_target_zone_from_slba_or_fail ${zone1_slba}
		return 0
...
function _zbc_test_count_zone_realms()
{
	...
	if [ ${zone_domains_device} -ne 0 -a ${nr_realms} -eq 0 ]; then
...
function zbc_test_is_found_realm_faulty()
{
	...
	for (( j=0 ; j<${realm_nr_domains} ; j++ )) ; do
		...
		for (( i=0 ; i<${_realm_len} ; i++ )) ; do
			zbc_test_get_target_zone_from_slba_or_fail ${_target_slba}
			if [ $? -ne 0 ]; then
				break
			fi
...
function zbc_test_check_sk_ascq()
{
	...
	if [[ "${alt_expected_sk}" != "" && "${sk}"  == "${alt_expected_sk}" &&
	     "${alt_expected_asc}" != "" && "${asc}" == "${alt_expected_asc}" ]]; then
		zbc_test_print_passed "$*"
		return
	fi
