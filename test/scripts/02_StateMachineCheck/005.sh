...
# Test print
echo "[TEST][${nr_test}][SWRZ][OPEN_ZONE][EMPTY_TO_EXPLICIT_OPEN][NO_ERROR] start"
...
######################## Get zone info ######################## 
# Get zone info
sudo zbc_test_report_zones -ro 3 ${device} > ${nr_test}_zone_info.log 2>&1
...
while read _line; do
    ...
    if [ ${zone_condition} = "0x3" -a ${start_lba} = "${test_zone_lba}" ]; then
        target_lba=$(( ${start_lba} ))
