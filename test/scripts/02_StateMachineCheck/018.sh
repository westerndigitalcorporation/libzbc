...
# Test print
echo "[TEST][${nr_test}][SWRZ][CLOSE_ZONE][EXPLICIT_OPEN_TO_CLOSE][NO_ERROR] start"
...
######################## Get zone info ######################## 
# Get zone info
sudo zbc_test_report_zones -v -ro 4 ${device} > ${nr_test}_zone_info.log 2>&1
...
while read _line; do
    ...
    if [ ${4} = "0x4" -a ${5} = "${test_zone_lba}" ]; then
        target_lba=$(( ${5} + 0 ))
