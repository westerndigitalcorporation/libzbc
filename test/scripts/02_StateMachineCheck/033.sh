...
# Test print
echo "[TEST][${nr_test}][SWRZ][OPEN_ZONE][ALL][IMPLICIT_OPEN_TO_IMPLICIT_OPEN][NO_ERROR] start"
...
######################## Get zone info ######################## 
# Get zone info
sudo zbc_test_report_zones -v -ro 2 ${device} > ${nr_test}_zone_info.log 2>&1
...
while read _line; do
    ...
    if [ ${4} = "0x2" -a ${5} = "${test_zone_lba}" ]; then
        target_lba=$(( ${5} + 0 ))
