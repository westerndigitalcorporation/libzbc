...
# Test print
echo "[TEST][${nr_test}][SWRZ][FINISH_ZONE][ALL][CLOSE_TO_FULL][NO_ERROR] start"
...
sudo zbc_test_write_zone -v ${device} ${test_zone_lba} >> ${nr_test}.log 2>&1
sudo zbc_test_close_zone -v ${device} ${test_zone_lba} >> ${nr_test}.log 2>&1
sudo zbc_test_finish_zone -v ${device} -1 >> ${nr_test}.log 2>&1
...
######################## Get zone info ######################## 
# Get zone info
sudo zbc_test_report_zones -v -ro 5 ${device} > ${nr_test}_zone_info.log 2>&1
...
while read _line; do
    ...
    if [ ${4} = "0xe" -a ${5} = "${test_zone_lba}" ]; then
        target_lba=$(( ${5} + 0 ))
