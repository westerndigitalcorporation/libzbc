...
# Like test 061 but with both zones filled
zbc_test_init $0 "READ sequential zones boundary violation (both zones filled)" $*
...
# Search target LBA
    				#XXX Why does the test zone need to be non-full?
...
zbc_test_run ${bin_path}/zbc_test_finish_zone -v ${device} $(( ${target_lba} + 1 ))
