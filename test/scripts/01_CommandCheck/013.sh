...
# Test print
echo "[TEST][${nr_test}][SWRZ][WRITE_ZONE][INSUFFICIENT_ZONE_RESOURCES] start"
...
while read _line; do
    ...
    if [ ${count} -eq $(( ${max_open} )) ]; then
        echo ${5} > test_zone_lba.txt
        break
    fi

    if [ ${3} = "0x2" ]; then
        sudo zbc_test_open_zone -v ${device} ${5} >> ${nr_test}.log 2>&1
        count=${count}+1
    fi
