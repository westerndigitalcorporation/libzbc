...
zbc_test_init $0 "ZONE ACTIVATE${zbc_test_actv_flags}: all CONV or SOBR to type ${test_actv_type:-"sequential"} (zone addressing)" $*
...
zbc_test_realm_boundaries_not_shifting_or_NA "${seq_type}"
...
start_smr_lba=$(zbc_realm_start ${seq_type})
...
zbc_test_run ${bin_path}/zbc_test_zone_activate -v ${zbc_test_actv_flags} -z \
			${device} ${start_smr_lba} ${nr_seq_zones} ${seq_type}
