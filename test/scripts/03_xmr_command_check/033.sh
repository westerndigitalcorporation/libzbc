...
zbc_test_init $0 "ZONE ACTIVATE${zbc_test_actv_flags}: all type ${test_deactv_type:-${ZT_SEQ}} to CONV or SOBR (zone addressing)" $*
...
zbc_test_search_realm_by_type_and_actv_or_NA "${seq_type}" "conv"
...
zbc_test_run ${bin_path}/zbc_test_zone_activate -v ${zbc_test_actv_flags} -z \
			${device} ${start_cmr_lba} ${nr_conv_zones} ${cmr_type}
