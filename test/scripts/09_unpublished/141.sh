...
zbc_test_init $0 "FINISH zone in EMPTY condition when max_open zones are EXP_OPEN" $*
...
if [ $? -ne 0 ]; then
	...
	zbc_test_check_no_sk_ascq "(${max_open}+1) * (seq_zone_type=${seq_zone_type} (${target_type}))"
    else
	# Zone type participates in the OZR protocol
	zbc_test_check_sk_ascq "(${max_open}+1) * (seq_zone_type=${seq_zone_type} (${target_type}))"
    fi
fi
