...
# Find the first SWP realm that is convertible to CMR
zbc_test_search_realm_by_type_and_cvt "0x3" "conv"
if [ $? -ne 0 ]; then
    zbc_test_print_not_applicable "No realm is currently SWP and convertible to CMR"
fi
...
if [ -z "${sk}" ]; then
    # Verify that no convertible SWP realms are present
    zbc_test_get_zone_realm_info
    zbc_test_search_realm_by_type_and_cvt "0x3" "conv"
    if [ $? -eq 0 ]; then
        sk=${realm_num}
