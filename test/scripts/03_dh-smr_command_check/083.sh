...
zbc_test_init $0 "ZONE ACTIVATE(16): all SMR to CMR (domain addressing)" $*
...
# Find the first SWP domain that is convertible to CMR
...
if [ -z "${sk}" ]; then
    # Verify that no convertible SWP domains are present
    zbc_test_get_cvt_domain_info
    zbc_test_search_domain_by_type_and_cvt "0x3" "conv"
    if [ $? -eq 0 ]; then
        sk=${domain_num}
