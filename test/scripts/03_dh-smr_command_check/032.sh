...
# Start testing
zbc_test_run ${bin_path}/zbc_test_zone_activate -v -z ${device} ${start_smr_lba} ${nr_seq_zones} ${smr_type}
...
if [ -z "${sk}" ]; then
    # Verify that no CMR realms that can be activated as SMR present
    zbc_test_get_zone_realm_info
    zbc_test_search_realm_by_type_and_actv "0x1|0x4" "seq"
    if [ $? -eq 0 ]; then
	sk=${realm_num}
