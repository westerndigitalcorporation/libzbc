...
zbc_test_init $0 "ZONE ACTIVATE(32): all SMR to CMR (zone addressing)" $*
...
# Start testing
zbc_test_run ${bin_path}/zbc_test_zone_activate -v -z -32 ${device} ${realm_seq_start} ${nr_seq_zones} ${cmr_type}

# Check result
zbc_test_get_sk_ascq
zbc_test_check_no_sk_ascq

# Verify that no SMR realm that can be activated as CMR is present
zbc_test_get_zone_realm_info
zbc_test_search_realm_by_type_and_actv "0x2|0x3" "conv"
if [ $? -eq 0 ]; then
    sk=${realm_num}
